...
class ContractController extends Controller
{
    ...
    public function show_adding($id=null)
    {
        ...
        if($id!=null)
        {
            ...
            if (count($user))
            {
                $driver = \App\Models\Driver::where('state', 'waiting')->where('group_id', $id)->get();
        ...
        else if(Auth::guard('admin')->user()->hasPermission('contract_manage'))
        {
            ...
            $driver = \App\Models\Driver::where('state', 'waiting')->get();
            $vechile = \App\Models\Vechile::where('state', 'waiting')->get();
            return view('contract.addContract', compact('driver', 'vechile', ));
        }
        return back();
    ...
    public function show_contracts($state)
    {

     

         if(Auth::guard('admin')->user()->hasPermission('contract_manage'))
        {
            $contracts = Contract::select(['id', 'contract_number', 'car_plate_number', 'add_by'])
            ->where('contract_status',$state=='valid'?'ساري':'لاغي')
        ...
        return view('contract.showContracts', compact('contracts','state'));
