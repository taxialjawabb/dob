...
{
    public function show_adding($id)
    {
        ...
        if ($this->hasPermissionData($id) == false)
            ...
            if (count($user))
            {
                ...
                $vechile = Vechile::where('state', 'waiting')->where('group_id', $id)->get();

                return view('groups.shared.contract.addContract', compact('driver', 'vechile','group' ));
            }
    ...
    public function end_contract_show($id)
    {
        ...
        if ($contract != null) {
            ...
            return view('groups.shared.contract.endContract', compact('contract_id'));
        }
        return back();
    ...
    public function end_contract_view(Request $req)
    {
        ...
        return view('groups.shared.contract.endContractDetails', compact('contract', 'contract_data', 'request'));
    }
    public function end_contract(Request $request)
    {
        ...
        $contract->date_ending = Carbon::now();
        ...
        $contract_data->date_ending = Carbon::now();
        $contract_data->save();
        $driver = Driver::find($contract->id_driver);
        $vechile = Vechile::find($contract->vechile_id);
        ...
        return redirect('shared/groups/show/contracts/'.$driver->group_id);
    ...
    public function extension_contract(Request $request)
    {
        ...
        $request->validate([
         ...
         $contract = Contract::with(['contract_data' => function ($query) {
        ...
        $contract_status_before = $contract->contract_end_datetime >= Carbon::now() ? 'ساري' : 'منتهي';
        ...
        if ($total > 0) {
            ...
            $boxVechile = new GroupsInternalBox();
            $boxVechile->foreign_type = 'driver';
            $boxVechile->group_id = $driver->group_id;
    ...
    public function save_data(Request $request)
    {
        ...
        if ($request->main_financial_total > 0) {
            ...
            $boxVechile->add_date = Carbon::now();
            $boxVechile->add_by = Auth::guard('admin')->user()->id;
            $driver->group_balance -= $request->main_financial_total;
    ...
    {
        $contract = Contract::where('id', $id)->with(['contract_data' => function ($query) {
            return $query->get();
        }])->first();

        return view('groups.shared.contract.showContractDetails', compact('contract'));
    }
    ...
    public function show_contracts($id)
    {
        ...
        if ($this->hasPermissionData($id) == false)
            ...
            if(count($user)>0)
            {
               ...
               $group=Group::find($id);
                ...
                $contracts =Contract::select(['id', 'contract_number', 'car_plate_number', 'add_by'])->with('added_by:id,name')
                ->whereIn('id_driver',$list)
                ->with(['contract_data' => function ($query) {
                    return $query->orderBy('id', 'desc')->get();
                }])->get();
            return view('groups.shared.contract.showContractsForGroup', compact('contracts','id','group'));
    ...
    public function view_contract(Request $request)
    {
        ...
        'email',
        */
        $group=Group::find($driver->group_id);
        $company =new Stakeholders();
        $company->name = $group->name;
        $company->commerical_register = $group->commercial_register;
        $company->record_number = $group->name;
        $company->id_number = $group->municipal_license_number;
        $company->license_number = $group->transportation_license_number;
        $company->license_category = $group->facility_type;
        $company->phone = $group->phone;
        $company->address = $group->city;
        $company->company_fax = "لايوجد";
        $company->email = "لايوجد";
        ...
        return view('groups.shared.contract.viewContract', compact('driver', 'vechile', 'company', 'request'));
