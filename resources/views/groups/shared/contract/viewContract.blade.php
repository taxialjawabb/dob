...
<input type="hidden" name="group_id" value="{{$driver->group_id}}" id="group_id">
<form method="POST" id="form-contract" action="{{ url('shared/groups/contract/adddata') }}">
    @csrf
      ...
      $("#user_confirm").click(function(e){
        ...
        $("#save-contract-final").click(function(e){
            ...
            if(confirmDriverCode && confirmUserCode){
                ...
                $.ajax({
                type: 'post',
                url: '{!!URL::to("shared/groups/contract/adddata")!!}',
                data: inp,
                success: function(data){
                    if(data.success === true){
                        ...
                        let url="shared/groups/show/contracts/"+groupId+"/valid";
                        window.location.href = `{!!URL::to('${url}')!!}`;
                    }
                },
                error:function(data_error, exception){
                    if(exception == 'error'){
                        $("#alert-danger").html('<ul></ul>')
                       var error_list = "";
                       $.each(data_error.responseJSON.errors, function(index, v){
                        error_list += "<li>"+ v +"</li>"
                       });
                        $("#alert-danger ul").html(error_list);
                    }
                    console.log('error');
                    console.log(e);
                }
        ...
        function sendCodeToUser(e){
            ...
            $.ajax({
                type: 'post',
                url: '{!!URL::to("shared/groups/user/contract/send/code")!!}',
                data: {
                        "_token": "{{ csrf_token() }}",
                        'id': userId,
        ...
        function sendCodeToDriver(e){
            ...
            $.ajax({
                type: 'post',
                url: '{!!URL::to("shared/groups/contract/send/code")!!}',
                data: {
                        "_token": "{{ csrf_token() }}",
                        'id': driverId,
