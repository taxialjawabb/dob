...
<div class="mt-2">
    ...
    <i class="arrow left "></i>
    <a  href="{{ url($driver->group_id === null? 'driver/details/' . $driver->id:'shared/groups/driver/detials/'.$driver->id .'/'. $driver->group_id) }}" class="btn text-primary p-1">{{$driver->name}}</a>
    <i class="arrow left "></i>
    <a href="{{ route('shared.groups.driver.box.show', [ 'driver'=> $driver->id, 'group'=>$group->id,'type' => 'take']) }}" class="btn text-primary p-1 ">الصندوق</a>

  </div>
<h5 class="mt-4">أضافة سند للسائق</h5>

<form  method="POST" action="{{ route('shared.groups.driver.box.save' , $driver) }}">
    @csrf
    <input type="hidden" id="stakeholder_id" name="foreign_type" value="driver" required>
    <input type="hidden" id="stakeholder_id" name="foreign_id" value="{{ $driver->id }}" required>
...
<script>
        ...
        $.ajax({
            ...
            data: {'from': 'driver', 'to':stakeholder, 'id': stakeholder_id},
