...
<div class="mt-2">
    ...
    <i class="arrow left "></i>
    <a  href="{{ url($vechile->group_id === null? 'vechile/details/' . $vechile->id:'shared/groups/vechile/detials/'.$vechile->id .'/'. $vechile->group_id) }}" class="btn text-primary p-1">{{$vechile->plate_number}}</a>
    <i class="arrow left "></i>
    <a href="{{ route('shared.groups.vechile.box.show', [ 'vechile'=> $vechile->id, 'group'=>$group->id,'type' => 'take']) }}" class="btn text-primary p-1 ">الصندوق</a>
</div>
<h5 class="mt-4">أضافة سند للمركبة</h5>

<form  method="POST" action="{{ route('shared.groups.vechile.box.save' , $vechile) }}">
    @csrf
    <input type="hidden" id="stakeholder_id" name="foreign_type" value="vechile" required>
    <input type="hidden" id="stakeholder_id" name="foreign_id" value="{{ $vechile->id }}" required>
        ...
        <div class="row" id="internal-transfer" style="display: none">
            <div class="mt-4  col-sm-12 col-md-6 col-lg-6 col-xl-6 ">
                ...
                <select value="{{ old('stakeholder') }}" name="stakeholder" id="stakeholder" class="form-select" aria-label="Default select example" id="stakeholder">
                    ...
                    <option value="vechile">السائقين</option>
