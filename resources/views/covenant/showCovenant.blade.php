...
@section('content')
    <div class="container clearfix">
        ...
        <div class="float-end mt-4">
            @if(Auth::user()->isAbleTo('covenant_add_new'))
            <a href="{{ url('covenant/add') }}" class="btn btn-success rounded-0 m-0">أضـافـة عهده جديد</a>
            @endif
            @if(Auth::user()->isAbleTo('covenant_deliver_to'))
            <a href="{{ url('covenant/delivering') }}"  class="btn btn-primary rounded-0 m-0">
                تسليم العهد لمستخدم
            </a>
            @endif
        </div>
                ...
                <tbody>
                    @foreach ($covenants as $covenant)
                        <tr class="covenat{{ $covenant->id }}">
                            ...
                            <td>
                            @if( $covenant->delivered_to !== null)
                                {{ $covenant->delivered_to->name}}
                            @endif
                            </td>
                           <td>{{ $covenant->added_by->name }}</td>
