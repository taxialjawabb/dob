...
@section('content')
    <div class="container clearfix">
        ...
        <div class="float-end mt-3">
            <a href="{{ url('vechile/box/show/take/' . $vechile->id) }}"
                class="btn {{ $type === 'take' ? 'btn-primary' : '' }} rounded-0 m-0">قـبـض</a>
            <a href="{{ url('vechile/box/show/spend/' . $vechile->id) }}"
                class="btn {{ $type === 'spend' ? 'btn-primary' : '' }} rounded-0 m-0">صــرف</a>
                @if(Auth::user()->isAbleTo('vechile_add_new_box'))
                ...
                <tbody>
                    @foreach ($bonds as $bond)
                        <tr class="print{{ $bond->id }}">
                            ...
                            @endif
                            <td>
                                @if(Auth::user()->isAbleTo('vechile_print_box'))
