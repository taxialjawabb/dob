...
<div class="container mt-4 clearfix">
    <h5 class="mt-1 float-start">فـواتـيـر بإنتظار التأكيد</h5>
    <div class="float-end">
        @if(Auth::user()->isAbleTo('bank_transfer_rider') || Auth::user()->isAbleTo('bank_transfer_driver'))
            <div class="dropdown float-start ms-1 ">
                ...
                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                    ...
                    @if(Auth::user()->isAbleTo('bank_transfer_rider'))
                    <li><a href="{{ url('bank/transfer/rider/show') }}" class="dropdown-item">التحويلات للعملاء</a></li>
                    @endif
                    @if(Auth::user()->isAbleTo('bank_transfer_driver'))
                    <li><a href="{{ url('bank/transfer/driver/show') }}" class="dropdown-item">التحويلات للسائقين</a></li>
                    @endif
                </ul>
            </div>
            @endif
                            ...
                            <table class="table " id="datatable">
                                ...
                                <tbody>
                                    ...
                                    <tr class="bill{{ ++$index }}">
                                        ...
                                        <td class="date">{{ $bill->bond_date }}</td>
...
<script>
    ...
    for(var i =0 ; i < listItem.length; i++){
        listItem[i].addEventListener('click', function(e){
                ...
                $.ajax({
                    type: 'post',
                    url: '{!!URL::to("bills/waiting/confrim")!!}',
                    data: {
                            "_token": "{{ csrf_token() }}",
                            'id':id,
                            'date':date,
    ...
    for(var i =0 ; i < listItem.length; i++){
        listItem[i].addEventListener('click', function(e){
                ...
                $.ajax({
                    type: 'post',
                    url: '{!!URL::to("bills/waiting/confrim/show")!!}',
                    data: {
                            "_token": "{{ csrf_token() }}",
                            'id':id,
                            'date':date,
                    ...
                    success: function(data){
                        ...
                        for (let index = 0; index < data.length; index++) {
                            ...
                            addBy = ( data[index].addBy !== null)?data[index].addBy: '';
                            htmlContent += `<tr>
