...
<div class="container clearfix">
    <h5 class=" mt-4 float-start">عرض السندات من النوع <span class="bond-type" id="{{ $type }}">{{ $type === 'take' ? 'قـبـض' : 'صــرف'
            }}</span></h5>

    <div class="float-end mt-3">
        <a href="{{ url('driver/box/show/take/' . $driver->id) }}"
            class="btn {{ $type === 'take' ? 'btn-primary' : '' }} rounded-0 m-0">قـبـض</a>
        <a href="{{ url('driver/box/show/spend/' . $driver->id) }}"
            class="btn {{ $type === 'spend' ? 'btn-primary' : '' }} rounded-0 m-0">صــرف</a>
        @if(Auth::user()->isAbleTo('driver_add_box'))
        <a href="{{ url('driver/box/add/' . $driver->id) }}" class="btn btn-success rounded-0 m-0">أضـافـة سند</a>
        @endif
    </div>
</div>
<div class="container">
    <div class="row">
        <div class="col-xs-12 col-5">
            <h6>
                الاسم : <span class="mt-2 bond-name">{{ $driver->name }}</span>
            </h6>
        </div>
        <div class="clearfix ">
            ...
            <div class="float-start text-center">
                ...
                <div class="bg-warning m-2 text-center p-2 " id="total-account">
                    {{ $driver->account }}
                </div>
            </div>
        ...
        <table class="table " id="datatable">
            ...
            <tbody>
                ...
                <tr class="print{{ $bond->id }}">
                    <td class="bond-state d-none" >{{ $bond->bond_state }}</td>
                    ...
                    @if($type === 'spend')
                    <td>
                        @if($bond->bond_photo !== null)
                        <form method="GET" action="{{ url('show/pdf') }}">
                            @csrf
                            <input type="hidden" name="url" value="{{'assets/images/bond/spend/'.$bond->bond_photo}}">
                    ...
                    @endif
                    <td>
                        @if(Auth::user()->isAbleTo('driver_print_box'))
                        <button id="print{{ $bond->id }}" type="button" class="btn btn-primary print"
                            data-bs-toggle="modal" data-bs-target="#exampleModal">طباعة</button>
                        @endif
                    </td>
                </tr>
                @endforeach
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="container-fluid mt-1 element-center" id="printable">
                <p class="mb-1 pt-1 element-center  text-center"
                    style="color: black; font-size: 22px; font-weight:bold">شركة الجواب للنقل البري </p>
                <p class="mb-1 pt-1 text-center " style="color: black; font-size: 16px;">فاتورة رقم * <span
                        id="bond-number"></span> * </p>
                <table class="table table-bordered element-center">
                    <thead>
                        <tr>
                            <th>
                                <p class="m-1" style="color: black; font-size: 16px;">اسم المستفيد</p>
                            </th>
                            <th colspan="3">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <p style="color: black; font-size: 18px;"> المبلغ المتبقى</p>
                            </td>
                            <td colspan="3">
                                <p style="color: black; font-size: 18px;">
                                    <span id="total-remains"></span> ريال سعودى
                                </p>
                            </td>
                        </tr>
                        <tr id="confirmed-by">
    ...
    $(function() {
            ...
            for (var i = 0; i < listItem.length; i++) {
                listItem[i].addEventListener('click', function(e) {
                    ...
                    var type = $(".bond-type").attr('id');
                    var bondState = $("." + e.target.id + " .bond-state").text();
                    ...
                    var totalRemains = $("#total-account").text();
                    if(bondState == 'waiting' || bondState == 'confirmed'){
                        if(type == 'take'){
                            totalRemains = Number(totalRemains) + Number(totalMoney);
                        }
                        else if(type == 'spend'){
                            totalRemains = Number(totalRemains) - Number(totalMoney) ;
                        }
                    ...
                    $("#total-remains").text(totalRemains);
