...
<div class="container clearfix">
    ...
    <div class="float-end mt-3">
        ...
        <a href="{{ route('driver.show.contracts','valid')}}" class="btn {{$state === 'valid'? 'btn-primary': ''}}">ساري</a>
        <a href="{{ route('driver.show.contracts','invalid')}}" class="btn {{$state === 'invalid'? 'btn-primary': ''}}">  منتهي</a>
                ...
                <tbody>
                    @foreach ($contracts as $contract)
                        <tr class="contract{{ $contract->id }}">
                            ...
                            <td>   <a href="{{url('driver/contract/show/details/'.$contract->id)}}" class="btn btn-primary rounded-1 m-0" >عرض</a></td>
                             @if ($contract->contract_data[0]->contract_status!='لاغي')
                            ...
                            @endif
                             <td><a href="{{ url('driver/contract/end/'.$contract->id) }}" class="btn btn-danger">انهاء تعاقد</a></td>
                            @endif
                            </tr>
                    @endforeach
                </tbody>
